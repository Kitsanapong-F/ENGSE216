...
public class List {
    ...
    public void bubBlesort() {
        for (int inDex = 0; inDex < count - 1; inDex++) {
            for (int inDexall = 0; inDexall < count - 1; inDexall++) {
                if ((this.conTact[inDexall].getName() + " " + this.conTact[inDexall].getLastname()).toUpperCase()
                        .compareTo(
                                (this.conTact[inDexall + 1].getName() + " " + this.conTact[inDexall + 1].getLastname())
                                        .toUpperCase()) > 0) {
    ...
    public void showAll() {
        for (int inDex = 0; inDex < count; inDex++) {
            System.out.println(this.conTact[inDex].getName() + " " + this.conTact[inDex].getLastname() + " "
                    + this.conTact[inDex].getPhon());
        }
    }
}
